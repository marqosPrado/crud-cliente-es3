...
async function searchClients(name, cpf, email, phone) {
    ...
    try {
        ...
        addClientsToTable(response.data);
    } catch (error) {
...
function addClientsToTable(clientes) {
    ...
    if (clientes.length > 0) {
        clientes.forEach(cliente => {
            ...
            estadoCell.textContent = cliente.enderecos[0]?.estado?.nome || '';

            const cidadeCell = document.createElement('td');
            cidadeCell.textContent = cliente.enderecos[0]?.cidade?.nome || '';

            const actionsCell = document.createElement('td');
            actionsCell.classList.add('row-icons');

            const inactiveButtonDiv = document.createElement('div');
            inactiveButtonDiv.classList.add('inactive-client-button', 'icons-container');
            inactiveButtonDiv.setAttribute('data-id', cliente.id);
            inactiveButtonDiv.innerHTML = `
                                <svg style="width: 32px; height: 32px;" xmlns="http://www.w3.org/2000/svg" x="0px" y="0px" width="100" height="100" viewBox="0 0 100 100">
            ...
            detailsDiv.classList.add('icons-container');

            const detailsLink = document.createElement('a');
            detailsLink.href = `${cliente.id}/detalhes`;
            detailsLink.innerHTML = `
                                    <svg style="width: 26px; height: 26px;" viewBox="0 0 1024 1024" class="icon"  version="1.1" xmlns="http://www.w3.org/2000/svg">
            ...
            actionsCell.appendChild(inactiveButtonDiv);
            actionsCell.appendChild(detailsDiv);
